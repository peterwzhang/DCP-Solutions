# DCP-Solutions
This repository is a collection of solutions for [Daily Coding Problem](https://www.dailycodingproblem.com) problems.  
"A daily coding problem a day keeps the unemployment away..."

## Authors
- **Madeline Moore** - [mcmoore11](https://github.com/mcmoore11)
    - told Peter about the DCPs... he had already been subscribed for over a year
    - managed all folders
- **Peter Zhang** - [PeterTheAmazingAsian](https://github.com/PeterTheAmazingAsian)
    - made the readme lol
    - subcribed to DCP for way too long

## Solutions
Below we have a table of all the DCPs (1-399). The table contains the question number/day, the company that asked the question, a brief description, difficulty, and a link to goto the corresponding folder. There are also two additional columns to show who has completed which problems and in which languages.
  
|Day| Company        | Description                    | Difficulty | Madeline   | Peter      | Link      |
|---|----------------| ------------------------------ | ---------: | ---------- | ---------- | :-------: |
|001| Google         | Given a list of numbers and a number k, return whether any two numbers from the list add up to k. | Easy       | Python |C++, Python| [go](solutions/001) |
|002| Uber           | Given an array of integers, return a new array such that each element at index i of the new array is the product of all the numbers in the original array except the one at i. | Hard       | Python |C++, Python| [go](solutions/002) |
|003| Google         | Given the root to a binary tree, implement serialize(root), which serializes the tree into a string, and deserialize(s), which deserializes the string back into the tree. | Medium     | -          |C++, Python| [go](solutions/003) |
|004| Stripe         | Given an array of integers, find the first missing positive integer in linear time and constant space. In other words, find the lowest positive integer that does not exist in the array. The array can contain duplicates and negative numbers as well. | Hard       | -          |C++, Python| [go](solutions/004) |
|005| Jane Street    | Implement car and cdr. cons(a, b) constructs a pair, and car(pair) and cdr(pair) returns the first and last element of that pair. | Medium     | -          | C++, Python | [go](solutions/005) |
|006| Google         | Implement an XOR linked list; it has an add(element) which adds the element to the end, and a get(index) which returns the node at index. | Hard       | -          |C++| [go](solutions/006) |
|007| Facebook       | Given the mapping a = 1, b = 2, ... z = 26, and an encoded message, count the number of ways it can be decoded. | Medium     | -          | -          | [go](solutions/007) |
|008| Google         | Given the root to a binary tree, count the number of unival subtrees. | Easy       | -          |C++| [go](solutions/008) |
|009| Airbnb         | Given a list of integers, write a function that returns the largest sum of non-adjacent numbers. Numbers can be 0 or negative. | Hard       | -          | -          | [go](solutions/009) |
|010| Apple          | Implement a job scheduler which takes in a function f and an integer n, and calls f after n milliseconds. | Medium     | -          | -          | [go](solutions/010) |
|011| Twitter        | Implement an autocomplete system. That is, given a query string s and a set of all possible query strings, return all strings in the set that have s as a prefix. | Medium     | -          | C++ | [go](solutions/011) |
|012| Amazon         | There exists a staircase with N steps, and you can climb up either 1 or 2 steps at a time. Given N, write a function that returns the number of unique ways you can climb the staircase. The order of the steps matters. | Hard       | -          | C++ | [go](solutions/012) |
|013| Amazon         | Given an integer k and a string s, find the length of the longest substring that contains at most k distinct characters. | Hard       | -          | -          | [go](solutions/013) |
|014| Google         | The area of a circle is defined as πr^2. Estimate π to 3 decimal places using a Monte Carlo method. | Medium     | -          | C++ | [go](solutions/014) |
|015| Facebook       | Given a stream of elements too large to store in memory, pick a random element from the stream with uniform probability. | Medium     | -          | -          | [go](solutions/015) |
|016| Twitter        | You run an e-commerce website and want to record the last N order ids in a log. Implement a data structure to accomplish this | Easy       | -          | -          | [go](solutions/016) |
|017| Google         | Given a string representing the file system in the above format, return the length of the longest absolute path to a file in the abstracted file system. If there is no file in the system, return 0. | Hard       | -          | -          | [go](solutions/017) |
|018| Google         | Given an array of integers and a number k, where 1 <= k <= length of the array, compute the maximum values of each subarray of length k. | Hard       | -          | -          | [go](solutions/018) |
|019| Facebook       | Goal of minimizing cost while ensuring that no two neighboring houses are of the same color. Given an N by K matrix where the nth row and kth column represents the cost to build the nthhouse with kth color, return the minimum cost which achieves this goal. | Medium     | -          | -          | [go](solutions/019) |
|020| Google         | Given two singly linked lists that intersect at some point, find the intersecting node. The lists are non-cyclical. | Easy       | -          | -          | [go](solutions/020) |
|021| Snapchat       | Given an array of time intervals (start, end) for classroom lectures (possibly overlapping), find the minimum number of rooms required. | Easy       | -          | -          | [go](solutions/021) |
|022| Microsoft      | Given a dictionary of words and a string made up of those words (no spaces), return the original sentence in a list. | Medium     | -          | C++ | [go](solutions/022) |
|023| Google         | You are given an M by N matrix consisting of booleans that represents a board. Each True boolean represents a wall. Each False boolean represents a tile you can walk on. Given this matrix, a start coordinate, and an end coordinate, return the minimum number of steps required to reach the end coordinate from the start. | Easy       | -          | -          | [go](solutions/023) |
|024| Google         | Implement locking in a binary tree. A binary tree node can be locked or unlocked only if all of its descendants or ancestors are not locked. | Medium     | -          | C++ | [go](solutions/024) |
|025| Facebook       | Implement regular expression matching with the following special characters: . and * | Hard       | -          | -          | [go](solutions/025) |
|026| Google         | -                              | Medium     | -          | -          | [go](solutions/026) |
|027| Facebook       | -                              | Easy       | -          | -          | [go](solutions/027) |
|028| Palantir       | -                              | Medium     | -          | -          | [go](solutions/028) |
|029| Amazon         | -                              | Easy       | -          | -          | [go](solutions/029) |
|030| Facebook       | -                              | Medium     | -          | -          | [go](solutions/030) |
|031| Google         | Given two strings, compute the edit distance between them. | Easy       | -          | C++ | [go](solutions/031) |
|032| Jane Street    | -                              | Hard       | -          | -          | [go](solutions/032) |
|033| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/033) |
|034| Quora          | -                              | Medium     | -          | -          | [go](solutions/034) |
|035| Google         | -                              | Hard       | -          | -          | [go](solutions/035) |
|036| Dropbox        | -                              | Medium     | -          | -          | [go](solutions/036) |
|037| Google         | The power set of a set is the set of all its subsets. Write a function that given a set, generates its power set. | Easy       | -          | C++ | [go](solutions/037) |
|038| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/038) |
|039| Dropbox        | -                              | Medium     | -          | -          | [go](solutions/039) |
|040| Google         | -                              | Hard       | -          | -          | [go](solutions/040) |
|041| Facebook       | -                              | Medium     | -          | -          | [go](solutions/041) |
|042| Google         | -                              | Hard       | -          | -          | [go](solutions/042) |
|043| Amazon         | -                              | Easy       | -          | -          | [go](solutions/043) |
|044| Google         | -                              | Medium     | -          | -          | [go](solutions/044) |
|045| Two Sigma      | -                              | Easy       | -          | -          | [go](solutions/045) |
|046| Amazon         | -                              | Hard       | -          | -          | [go](solutions/046) |
|047| Facebook       | -                              | Easy       | -          | -          | [go](solutions/047) |
|048| Google         | -                              | Medium     | -          | -          | [go](solutions/048) |
|049| Amazon         | Given an array of numbers, find the maximum sum of any contiguous subarray of the array. | Medium     | -          | C++ | [go](solutions/049) |
|050| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/050) |
|051| Facebook       | -                              | Medium     | -          | -          | [go](solutions/051) |
|052| Google         | -                              | Hard       | -          | -          | [go](solutions/052) |
|053| Apple          | Implement a queue using two stacks. | Medium     | -          | C++ | [go](solutions/053) |
|054| Dropbox        | -                              | Hard       | -          | -          | [go](solutions/054) |
|055| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/055) |
|056| Google         | -                              | Medium     | -          | -          | [go](solutions/056) |
|057| Amazon         | -                              | Medium     | -          | -          | [go](solutions/057) |
|058| Amazon         | Given such an array, find the index of the element in the array in faster than linear time. | Medium     | -          | C++ | [go](solutions/058) |
|059| Google         | -                              | Hard       | -          | -          | [go](solutions/059) |
|060| Facebook       | -                              | Medium     | -          | -          | [go](solutions/060) |
|061| Google         | -                              | Medium     | -          | -          | [go](solutions/061) |
|062| Facebook       | -                              | Medium     | -          | -          | [go](solutions/062) |
|063| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/063) |
|064| Google         | -                              | Hard       | -          | -          | [go](solutions/064) |
|065| Amazon         | Given a N by M matrix of numbers, print out the matrix in a clockwise spiral. | Easy       | -          | C++ | [go](solutions/065) |
|066| Square         | -                              | Medium     | -          | -          | [go](solutions/066) |
|067| Google         | -                              | Hard       | -          | -          | [go](solutions/067) |
|068| Google         | -                              | Medium     | -          | -          | [go](solutions/068) |
|069| Facebook       | Given a list of integers, return the largest product that can be made by multiplying any three integers. | Easy       | -          | C++ | [go](solutions/069) |
|070| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/070) |
|071| Two Sigma      | -                              | Easy       | -          | -          | [go](solutions/071) |
|072| Google         | -                              | Hard       | -          | -          | [go](solutions/072) |
|073| Google         | -                              | Easy       | -          | -          | [go](solutions/073) |
|074| Apple          | Given integers N and X, write a function that returns the number of times X appears as a value in an N by N multiplication table. | Medium     | -          | C++ | [go](solutions/074) |
|075| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/075) |
|076| Google         | -                              | Medium     | -          | -          | [go](solutions/076) |
|077| Snapchat       | -                              | Easy       | -          | -          | [go](solutions/077) |
|078| Google         | -                              | Medium     | -          | -          | [go](solutions/078) |
|079| Facebook       | -                              | Medium     | -          | -          | [go](solutions/079) |
|080| Google         | -                              | Easy       | -          | -          | [go](solutions/080) |
|081| Yelp           | -                              | Easy       | -          | -          | [go](solutions/081) |
|082| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/082) |
|083| Google         | -                              | Medium     | -          | -          | [go](solutions/083) |
|084| Amazon         | -                              | Medium     | -          | -          | [go](solutions/084) |
|085| Facebook       | -                              | Medium     | -          | -          | [go](solutions/085) |
|086| Google         | -                              | Medium     | -          | -          | [go](solutions/086) |
|087| Uber           | -                              | Hard       | -          | -          | [go](solutions/087) |
|088| ContextLogic   | -                              | Medium     | -          | -          | [go](solutions/088) |
|089| LinkedIn       | -                              | Medium     | -          | -          | [go](solutions/089) |
|090| Google         | -                              | Medium     | -          | -          | [go](solutions/090) |
|091| Dropbox        | Fix the code snippet to print 0-9 | Easy       | -          | Python | [go](solutions/091) |
|092| Airbnb         | -                              | Hard       | -          | -          | [go](solutions/092) |
|093| Apple          | -                              | Hard       | -          | -          | [go](solutions/093) |
|094| Google         | -                              | Easy       | -          | -          | [go](solutions/094) |
|095| Palantir       | -                              | Hard       | -          | -          | [go](solutions/095) |
|096| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/096) |
|097| Stripe         | -                              | Medium     | -          | -          | [go](solutions/097) |
|098| Coursera       | -                              | Easy       | -          | -          | [go](solutions/098) |
|099| Microsoft      | -                              | Medium     | -          | -          | [go](solutions/099) |
|100| Google         | -                              | Easy       | -          | -          | [go](solutions/100) |
|101| Alibaba        | -                              | Easy       | -          | -          | [go](solutions/101) |
|102| Lyft           | -                              | Medium     | -          | -          | [go](solutions/102) |
|103| Square         | -                              | Medium     | -          | -          | [go](solutions/103) |
|104| Google         | -                              | Easy       | -          | -          | [go](solutions/104) |
|105| Facebook       | -                              | Easy       | -          | -          | [go](solutions/105) |
|106| Pinterest      | -                              | Medium     | -          | -          | [go](solutions/106) |
|107| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/107) |
|108| Google         | -                              | Easy       | -          | -          | [go](solutions/108) |
|109| Cisco          | -                              | Medium     | -          | -          | [go](solutions/109) |
|110| Facebook       | -                              | Medium     | -          | -          | [go](solutions/110) |
|111| Google         | -                              | Hard       | -          | -          | [go](solutions/111) |
|112| Twitter        | -                              | Hard       | -          | -          | [go](solutions/112) |
|113| Google         | -                              | Medium     | -          | -          | [go](solutions/113) |
|114| Facebook       | -                              | Hard       | -          | -          | [go](solutions/114) |
|115| Google         | -                              | Hard       | -          | -          | [go](solutions/115) |
|116| Jane Street    | -                              | Medium     | -          | -          | [go](solutions/116) |
|117| Facebook       | -                              | Easy       | -          | -          | [go](solutions/117) |
|118| Google         | -                              | Easy       | -          | -          | [go](solutions/118) |
|119| Google         | -                              | Medium     | -          | -          | [go](solutions/119) |
|120| Microsoft      | -                              | Medium     | -          | -          | [go](solutions/120) |
|121| Google         | -                              | Hard       | -          | -          | [go](solutions/121) |
|122| Zillow         | -                              | Medium     | -          | -          | [go](solutions/122) |
|123| LinkedIn       | -                              | Hard       | -          | -          | [go](solutions/123) |
|124| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/124) |
|125| Google         | -                              | Easy       | -          | -          | [go](solutions/125) |
|126| Facebook       | -                              | Medium     | -          | -          | [go](solutions/126) |
|127| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/127) |
|128| None           | -                              | Medium     | -          | -          | [go](solutions/128) |
|129| None           | -                              | Medium     | -          | -          | [go](solutions/129) |
|130| Facebook       | -                              | Medium     | -          | -          | [go](solutions/130) |
|131| Snapchat       | -                              | Medium     | -          | -          | [go](solutions/131) |
|132| Riot Games     | -                              | Easy       | -          | -          | [go](solutions/132) |
|133| Amazon         | -                              | Medium     | -          | -          | [go](solutions/133) |
|134| Facebook       | -                              | Easy       | -          | -          | [go](solutions/134) |
|135| Apple          | -                              | Easy       | -          | -          | [go](solutions/135) |
|136| Google         | -                              | Medium     | -          | -          | [go](solutions/136) |
|137| Amazon         | -                              | Medium     | -          | -          | [go](solutions/137) |
|138| Google         | -                              | Hard       | -          | -          | [go](solutions/138) |
|139| Google         | -                              | Medium     | -          | -          | [go](solutions/139) |
|140| Facebook       | -                              | Medium     | -          | -          | [go](solutions/140) |
|141| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/141) |
|142| Google         | -                              | Hard       | -          | -          | [go](solutions/142) |
|143| Amazon         | -                              | Medium     | -          | -          | [go](solutions/143) |
|144| Google         | -                              | Medium     | -          | -          | [go](solutions/144) |
|145| Google         | -                              | Easy       | -          | -          | [go](solutions/145) |
|146| BufferBox      | -                              | Medium     | -          | -          | [go](solutions/146) |
|147| None           | -                              | Hard       | -          | -          | [go](solutions/147) |
|148| Apple          | -                              | Medium     | -          | -          | [go](solutions/148) |
|149| Goldman Sachs  | -                              | Hard       | -          | -          | [go](solutions/149) |
|150| LinkedIn       | -                              | Hard       | -          | -          | [go](solutions/150) |
|151| None           | -                              | Medium     | -          | -          | [go](solutions/151) |
|152| Triplebyte     | -                              | Medium     | -          | -          | [go](solutions/152) |
|153| None           | -                              | Hard       | -          | -          | [go](solutions/153) |
|154| Amazon         | -                              | Easy       | -          | -          | [go](solutions/154) |
|155| MongoDB        | -                              | Medium     | -          | -          | [go](solutions/155) |
|156| Facebook       | -                              | Medium     | -          | -          | [go](solutions/156) |
|157| Amazon         | -                              | Easy       | -          | -          | [go](solutions/157) |
|158| Slack          | -                              | Medium     | -          | -          | [go](solutions/158) |
|159| Google         | Given a string, return the first recurring character in it, or null if there is no recurring character. | Easy       | -          | C++ | [go](solutions/159) |
|160| Uber           | -                              | Hard       | -          | -          | [go](solutions/160) |
|161| Facebook       | -                              | Easy       | -          | -          | [go](solutions/161) |
|162| Square         | -                              | Medium     | -          | -          | [go](solutions/162) |
|163| Jane Street    | -                              | Hard       | -          | -          | [go](solutions/163) |
|164| Google         | -                              | Medium     | -          | -          | [go](solutions/164) |
|165| Google         | -                              | Medium     | -          | -          | [go](solutions/165) |
|166| Uber           | -                              | Medium     | -          | -          | [go](solutions/166) |
|167| Airbnb         | -                              | Hard       | -          | -          | [go](solutions/167) |
|168| Facebook       | -                              | Medium     | -          | -          | [go](solutions/168) |
|169| Google         | -                              | Medium     | -          | -          | [go](solutions/169) |
|170| Facebook       | -                              | Medium     | -          | -          | [go](solutions/170) |
|171| Amazon         | -                              | Easy       | -          | -          | [go](solutions/171) |
|172| Dropbox        | -                              | Medium     | -          | -          | [go](solutions/172) |
|173| Stripe         | -                              | Easy       | -          | -          | [go](solutions/173) |
|174| Microsoft      | -                              | Medium     | -          | -          | [go](solutions/174) |
|175| Google         | -                              | Easy       | -          | -          | [go](solutions/175) |
|176| Bloomberg      | -                              | Easy       | -          | -          | [go](solutions/176) |
|177| Airbnb         | -                              | Easy       | -          | -          | [go](solutions/177) |
|178| Two Sigma      | Find the expected values of 5->6 and 5->5 dice games. | Hard       | -          | C++ | [go](solutions/178) |
|179| Google         | -                              | Medium     | -          | -          | [go](solutions/179) |
|180| Google         | Given a stack of N elements, interleave the first half of the stack with the second half reversed using only one other queue. This should be done in-place. | Medium     | -          | C++ | [go](solutions/180) |
|181| Google         | -                              | Hard       | -          | -          | [go](solutions/181) |
|182| Facebook       | -                              | Medium     | -          | -          | [go](solutions/182) |
|183| Twitch         | -                              | Hard       | -          | -          | [go](solutions/183) |
|184| Amazon         | -                              | Easy       | -          | -          | [go](solutions/184) |
|185| Google         | -                              | Easy       | -          | -          | [go](solutions/185) |
|186| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/186) |
|187| Google         | -                              | Easy       | -          | -          | [go](solutions/187) |
|188| Google         | -                              | Medium     | -          | -          | [go](solutions/188) |
|189| Google         | Given an array of elements, return the length of the longest subarray where all its elements are distinct. | Easy       | -          | C++ | [go](solutions/189) |
|190| Facebook       | Given a circular array, compute its maximum subarray sum in O(n) time. A subarray can be empty, and in this case the sum is 0. | Medium     | -          | C++ | [go](solutions/190) |
|191| Stripe         | -                              | Easy       | -          | -          | [go](solutions/191) |
|192| Google         | -                              | Medium     | -          | -          | [go](solutions/192) |
|193| Affirm         | -                              | Hard       | -          | -          | [go](solutions/193) |
|194| Facebook       | -                              | Easy       | -          | -          | [go](solutions/194) |
|195| Google         | -                              | Hard       | -          | -          | [go](solutions/195) |
|196| Apple          | -                              | Easy       | -          | -          | [go](solutions/196) |
|197| Amazon         | -                              | Easy       | -          | -          | [go](solutions/197) |
|198| Google         | -                              | Medium     | -          | -          | [go](solutions/198) |
|199| Facebook       | -                              | Hard       | -          | -          | [go](solutions/199) |
|200| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/200) |
|201| Google         | -                              | Easy       | -          | -          | [go](solutions/201) |
|202| Palantir       | -                              | Easy       | -          | -          | [go](solutions/202) |
|203| Uber           | -                              | Medium     | -          | -          | [go](solutions/203) |
|204| Amazon         | -                              | Easy       | -          | -          | [go](solutions/204) |
|205| IBM            | -                              | Easy       | -          | -          | [go](solutions/205) |
|206| Twitter        | Given an array and a permutation, apply the permutation to the array. | Easy       | -          | C++ | [go](solutions/206) |
|207| Dropbox        | -                              | Medium     | -          | -          | [go](solutions/207) |
|208| LinkedIn       | -                              | Medium     | -          | -          | [go](solutions/208) |
|209| YouTube        | -                              | Hard       | -          | -          | [go](solutions/209) |
|210| Apple          | -                              | Easy       | -          | -          | [go](solutions/210) |
|211| Microsoft      | -                              | Medium     | -          | -          | [go](solutions/211) |
|212| Dropbox        | -                              | Easy       | -          | -          | [go](solutions/212) |
|213| Snapchat       | -                              | Medium     | -          | -          | [go](solutions/213) |
|214| Stripe         | Given an integer n, return the length of the longest consecutive run of 1s in its binary representation. | Easy       | -          | C++ | [go](solutions/214) |
|215| Yelp           | -                              | Medium     | -          | -          | [go](solutions/215) |
|216| Facebook       | -                              | Medium     | -          | -          | [go](solutions/216) |
|217| Oracle         | -                              | Hard       | -          | -          | [go](solutions/217) |
|218| Yahoo          | -                              | Medium     | -          | -          | [go](solutions/218) |
|219| Salesforce     | -                              | Hard       | -          | -          | [go](solutions/219) |
|220| Square         | -                              | Medium     | -          | -          | [go](solutions/220) |
|221| Zillow         | -                              | Easy       | -          | -          | [go](solutions/221) |
|222| Quora          | -                              | Medium     | -          | -          | [go](solutions/222) |
|223| Palantir       | -                              | Hard       | -          | -          | [go](solutions/223) |
|224| Amazon         | -                              | Easy       | -          | -          | [go](solutions/224) |
|225| Bloomberg      | -                              | Easy       | -          | -          | [go](solutions/225) |
|226| Airbnb         | -                              | Hard       | -          | -          | [go](solutions/226) |
|227| Facebook       | -                              | Easy       | -          | -          | [go](solutions/227) |
|228| Twitter        | -                              | Medium     | -          | -          | [go](solutions/228) |
|229| Flipkart       | -                              | Medium     | -          | -          | [go](solutions/229) |
|230| Goldman Sachs  | -                              | Medium     | -          | -          | [go](solutions/230) |
|231| IBM            | -                              | Easy       | -          | -          | [go](solutions/231) |
|232| Google         | -                              | Easy       | -          | -          | [go](solutions/232) |
|233| Apple          | Implement the function fib(n), which returns the nth number in the Fibonacci sequence, using only O(1) space. | Easy       | -          | C++ | [go](solutions/233) |
|234| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/234) |
|235| Facebook       | -                              | Hard       | -          | -          | [go](solutions/235) |
|236| Nvidia         | -                              | Medium     | -          | -          | [go](solutions/236) |
|237| Amazon         | -                              | Easy       | -          | -          | [go](solutions/237) |
|238| MIT            | -                              | Hard       | -          | -          | [go](solutions/238) |
|239| Uber           | -                              | Medium     | -          | -          | [go](solutions/239) |
|240| Spotify        | -                              | Hard       | -          | -          | [go](solutions/240) |
|241| Palantir       | -                              | Easy       | -          | -          | [go](solutions/241) |
|242| Twitter        | -                              | Hard       | -          | -          | [go](solutions/242) |
|243| Etsy           | -                              | Medium     | -          | -          | [go](solutions/243) |
|244| Square         | -                              | Easy       | -          | -          | [go](solutions/244) |
|245| Yelp           | -                              | Medium     | -          | -          | [go](solutions/245) |
|246| Dropbox        | -                              | Medium     | -          | -          | [go](solutions/246) |
|247| PayPal         | -                              | Easy       | -          | -          | [go](solutions/247) |
|248| Nvidia         | -                              | Hard       | -          | -          | [go](solutions/248) |
|249| Salesforce     | -                              | Hard       | -          | -          | [go](solutions/249) |
|250| Google         | -                              | Medium     | -          | -          | [go](solutions/250) |
|251| Amazon         | -                              | Medium     | -          | -          | [go](solutions/251) |
|252| Palantir       | -                              | Easy       | -          | -          | [go](solutions/252) |
|253| PayPal         | -                              | Medium     | -          | -          | [go](solutions/253) |
|254| Yahoo          | -                              | Medium     | -          | -          | [go](solutions/254) |
|255| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/255) |
|256| Fitbit         | -                              | Medium     | -          | -          | [go](solutions/256) |
|257| WhatsApp       | -                              | Easy       | -          | -          | [go](solutions/257) |
|258| Morgan Stanley | -                              | Easy       | -          | -          | [go](solutions/258) |
|259| Two Sigma      | -                              | Hard       | -          | -          | [go](solutions/259) |
|260| Pinterest      | -                              | Medium     | -          | -          | [go](solutions/260) |
|261| Amazon         | -                              | Easy       | -          | -          | [go](solutions/261) |
|262| Mozilla        | -                              | Medium     | -          | -          | [go](solutions/262) |
|263| Nest           | -                              | Medium     | -          | -          | [go](solutions/263) |
|264| LinkedIn       | -                              | Hard       | -          | -          | [go](solutions/264) |
|265| Atlassian      | -                              | Easy       | -          | -          | [go](solutions/265) |
|266| Pivotal        | -                              | Easy       | -          | -          | [go](solutions/266) |
|267| Oracle         | -                              | Hard       | -          | -          | [go](solutions/267) |
|268| Indeed         | -                              | Medium     | -          | -          | [go](solutions/268) |
|269| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/269) |
|270| Twitter        | -                              | Medium     | -          | -          | [go](solutions/270) |
|271| Netflix        | -                              | Hard       | -          | -          | [go](solutions/271) |
|272| Spotify        | -                              | Medium     | -          | -          | [go](solutions/272) |
|273| Apple          | A fixed point in an array is an element whose value is equal to its index. Given a sorted array of distinct elements, return a fixed point, if one exists. Otherwise, return False. | Easy       | -          |C++| [go](solutions/273) |
|274| Facebook       | -                              | Hard       | -          | -          | [go](solutions/274) |
|275| Epic           | -                              | Medium     | -          | -          | [go](solutions/275) |
|276| Dropbox        | -                              | Hard       | -          | -          | [go](solutions/276) |
|277| Google         | -                              | Easy       | -          | -          | [go](solutions/277) |
|278| Amazon         | -                              | Easy       | -          | -          | [go](solutions/278) |
|279| Twitter        | -                              | Easy       | -          | -          | [go](solutions/279) |
|280| Pandora        | -                              | Easy       | -          | -          | [go](solutions/280) |
|281| LinkedIn       | -                              | Medium     | -          | -          | [go](solutions/281) |
|282| Netflix        | -                              | Easy       | -          | -          | [go](solutions/282) |
|283| Google         | -                              | Easy       | -          | -          | [go](solutions/283) |
|284| Yext           | -                              | Medium     | -          | -          | [go](solutions/284) |
|285| Mailchimp      | -                              | Medium     | -          | -          | [go](solutions/285) |
|286| VMware         | -                              | Hard       | -          | -          | [go](solutions/286) |
|287| Quora          | -                              | Medium     | -          | -          | [go](solutions/287) |
|288| Salesforce     | -                              | Medium     | -          | -          | [go](solutions/288) |
|289| Google         | -                              | Hard       | -          | -          | [go](solutions/289) |
|290| Facebook       | -                              | Easy       | -          | -          | [go](solutions/290) |
|291| Glassdoor      | -                              | Medium     | -          | -          | [go](solutions/291) |
|292| Twitter        | -                              | Hard       | -          | -          | [go](solutions/292) |
|293| Uber           | -                              | Hard       | -          | -          | [go](solutions/293) |
|294| Square         | -                              | Medium     | -          | -          | [go](solutions/294) |
|295| Stitch Fix     | -                              | Medium     | -          | -          | [go](solutions/295) |
|296| Etsy           | -                              | Hard       | -          | -          | [go](solutions/296) |
|297| Amazon         | -                              | Medium     | -          | -          | [go](solutions/297) |
|298| Google         | -                              | Easy       | -          | -          | [go](solutions/298) |
|299| Samsung        | -                              | Medium     | -          | -          | [go](solutions/299) |
|300| Uber           | -                              | Easy       | -          | -          | [go](solutions/300) |
|301| Triplebyte     | -                              | Medium     | -          | -          | [go](solutions/301) |
|302| Uber           | -                              | Medium     | -          | -          | [go](solutions/302) |
|303| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/303) |
|304| Two Sigma      | -                              | Hard       | -          | -          | [go](solutions/304) |
|305| Amazon         | -                              | Easy       | -          | -          | [go](solutions/305) |
|306| Palantir       | -                              | Medium     | -          | -          | [go](solutions/306) |
|307| Oracle         | -                              | Easy       | -          | -          | [go](solutions/307) |
|308| Quantcast      | -                              | Hard       | -          | -          | [go](solutions/308) |
|309| Walmart Labs   | -                              | Medium     | -          | -          | [go](solutions/309) |
|310| Pivotal        | -                              | Easy       | -          | -          | [go](solutions/310) |
|311| Sumo Logic     | Given an unsorted array, in which all elements are distinct, find a "peak" element in O(log N) time. | Easy       | -          | C++ | [go](solutions/311) |
|312| Wayfair        | -                              | Easy       | -          | -          | [go](solutions/312) |
|313| Citrix         | -                              | Hard       | -          | -          | [go](solutions/313) |
|314| Spotify        | -                              | Medium     | -          | -          | [go](solutions/314) |
|315| Google         | -                              | Easy       | -          | -          | [go](solutions/315) |
|316| Snapchat       | -                              | Medium     | -          | -          | [go](solutions/316) |
|317| Yahoo          | -                              | Medium     | -          | -          | [go](solutions/317) |
|318| Apple          | -                              | Hard       | -          | -          | [go](solutions/318) |
|319| Airbnb         | -                              | Hard       | -          | -          | [go](solutions/319) |
|320| Amazon         | -                              | Medium     | -          | -          | [go](solutions/320) |
|321| PagerDuty      | -                              | Easy       | -          | -          | [go](solutions/321) |
|322| Flipkart       | -                              | Medium     | -          | -          | [go](solutions/322) |
|323| Dropbox        | -                              | Medium     | -          | -          | [go](solutions/323) |
|324| Amazon         | -                              | Easy       | -          | -          | [go](solutions/324) |
|325| Jane Street    | -                              | Easy       | -          | -          | [go](solutions/325) |
|326| Netflix        | -                              | Hard       | -          | -          | [go](solutions/326) |
|327| Salesforce     | -                              | Easy       | -          | -          | [go](solutions/327) |
|328| Facebook       | -                              | Medium     | -          | -          | [go](solutions/328) |
|329| Amazon         | -                              | Hard       | -          | -          | [go](solutions/329) |
|330| Dropbox        | -                              | Hard       | -          | -          | [go](solutions/330) |
|331| LinkedIn       | -                              | Medium     | -          | -          | [go](solutions/331) |
|332| Jane Street    | -                              | Easy       | -          | -          | [go](solutions/332) |
|333| Pinterest      | -                              | Medium     | -          | -          | [go](solutions/333) |
|334| Twitter        | Write a function that plays the 24 game. | Easy       | -          | C++ | [go](solutions/334) |
|335| Google         | -                              | Hard       | -          | -          | [go](solutions/335) |
|336| Microsoft      | -                              | Medium     | -          | -          | [go](solutions/336) |
|337| Apple          | -                              | Hard       | -          | -          | [go](solutions/337) |
|338| Facebook       | -                              | Medium     | -          | -          | [go](solutions/338) |
|339| Microsoft      | -                              | Easy       | -          | -          | [go](solutions/339) |
|340| Google         | -                              | Easy       | -          | -          | [go](solutions/340) |
|341| Google         | -                              | Easy       | -          | -          | [go](solutions/341) |
|342| Stripe         | -                              | Medium     | -          | -          | [go](solutions/342) |
|343| Google         | -                              | Medium     | -          | -          | [go](solutions/343) |
|344| Adobe          | -                              | Hard       | -          | -          | [go](solutions/344) |
|345| Google         | -                              | Medium     | -          | -          | [go](solutions/345) |
|346| Airbnb         | -                              | Medium     | -          | -          | [go](solutions/346) |
|347| Yahoo          | You are given a string of length N and a parameter k. The string can be manipulated by taking one of the first k letters and moving it to the end. | Easy       | -          | C++ | [go](solutions/347) |
|348| Zillow         | -                              | Easy       | -          | -          | [go](solutions/348) |
|349| Grammarly      | -                              | Hard       | -          | -          | [go](solutions/349) |
|350| Uber           | -                              | Medium     | -          | -          | [go](solutions/350) |
|351| Quora          | -                              | Hard       | -          | -          | [go](solutions/351) |
|352| Palantir       | -                              | Easy       | -          | -          | [go](solutions/352) |
|353| Square         | -                              | Medium     | -          | -          | [go](solutions/353) |
|354| Google         | -                              | Hard       | -          | -          | [go](solutions/354) |
|355| Airbnb         | -                              | Hard       | -          | -          | [go](solutions/355) |
|356| Netflix        | -                              | Hard       | -          | -          | [go](solutions/356) |
|357| LinkedIn       | -                              | Hard       | -          | -          | [go](solutions/357) |
|358| Dropbox        | -                              | Hard       | -          | -          | [go](solutions/358) |
|359| Slack          | -                              | Easy       | -          | -          | [go](solutions/359) |
|360| Spotify        | -                              | Medium     | -          | -          | [go](solutions/360) |
|361| Facebook       | -                              | Medium     | -          | -          | [go](solutions/361) |
|362| Twitter        | -                              | Easy       | -          | -          | [go](solutions/362) |
|363| Squarespace    | -                              | Medium     | -          | -          | [go](solutions/363) |
|364| Facebook       | -                              | Medium     | -          | -          | [go](solutions/364) |
|365| Google         | -                              | Hard       | -          | -          | [go](solutions/365) |
|366| Flexport       | -                              | Medium     | -          | -          | [go](solutions/366) |
|367| Two Sigma      | -                              | Medium     | -          | -          | [go](solutions/367) |
|368| Google         | -                              | Hard       | -          | -          | [go](solutions/368) |
|369| Two Sigma      | -                              | Medium     | -          | -          | [go](solutions/369) |
|370| Postmates      | -                              | Easy       | -          | -          | [go](solutions/370) |
|371| Google         | -                              | Hard       | -          | -          | [go](solutions/371) |
|372| Amazon         | Write a function that takes a natural number as input and returns the number of digits the input has. Don't use any loops. | Easy       | -          | C++ | [go](solutions/372) |
|373| Facebook       | -                              | Hard       | -          | -          | [go](solutions/373) |
|374| Amazon         | -                              | Hard       | -          | -          | [go](solutions/374) |
|375| Google         | -                              | Medium     | -          | -          | [go](solutions/375) |
|376| Google         | -                              | Easy       | -          | -          | [go](solutions/376) |
|377| Microsoft      | -                              | Hard       | -          | -          | [go](solutions/377) |
|378| Coinbase       | -                              | Medium     | -          | -          | [go](solutions/378) |
|379| Microsoft      | Given a string, generate all possible subsequences of the string. | Easy       | -          | C++ | [go](solutions/379) |
|380| Nextdoor       | -                              | Medium     | -          | -          | [go](solutions/380) |
|381| Paypal         | -                              | Easy       | -          | -          | [go](solutions/381) |
|382| Google         | -                              | Easy       | -          | -          | [go](solutions/382) |
|383| Gusto          | -                              | Medium     | -          | -          | [go](solutions/383) |
|384| WeWork         | -                              | Hard       | -          | -          | [go](solutions/384) |
|385| Apple          | -                              | Medium     | -          | -          | [go](solutions/385) |
|386| Twitter        | -                              | Easy       | -          | -          | [go](solutions/386) |
|387| Amazon         | -                              | Medium     | -          | -          | [go](solutions/387) |
|388| Airtable       | -                              | Medium     | -          | -          | [go](solutions/388) |
|389| Google         | -                              | Hard       | -          | -          | [go](solutions/389) |
|390| Two Sigma      | -                              | Medium     | -          | -          | [go](solutions/390) |
|391| Facebook       | -                              | Hard       | -          | -          | [go](solutions/391) |
|392| Google         | -                              | Hard       | -          | -          | [go](solutions/392) |
|393| Airbnb         | -                              | Medium     | -          | -          | [go](solutions/393) |
|394| Uber           | Given a binary tree and an integer k, return whether there exists a root-to-leaf path that sums up to k. | Easy       | -          | C++ | [go](solutions/394) |
|395| Robinhood      | -                              | Medium     | -          | -          | [go](solutions/395) |
|396| Google         | -                              | Hard       | -          | -          | [go](solutions/396) |
|397| Microsoft      | -                              | Medium     | -          | -          | [go](solutions/397) |
|398| Amazon         | -                              | Medium     | -          | -          | [go](solutions/398) |
|399| Facebook       | -                              | Hard       | -          | -          | [go](solutions/399) |